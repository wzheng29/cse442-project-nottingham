...
def getPrice(tag):
    ...
    frame = pdr.DataReader(ticker, 'yahoo', startDate-relativedelta(years=5), endDate-relativedelta(years=5))
...
def Plotter ( tag, start):
    ...
    spy = pdr.DataReader(tag,'yahoo', startDate-relativedelta(years=5), endDate)
    close = spy['Adj Close']
    msft = close.loc[:]
